...
(function attachAiModelVisual(root) {
    ...
    const ICON_SLUGS = { mimo:'xiaomimimo',glm:'zhipu',llama:'meta',siliconflow:'siliconcloud',yi:'zeroone' };
    ...
    function resolve({modelId='',provider='',iconKey='',dark,local}={}){ const found=detect(modelId); const raw=String(iconKey||'').trim().toLowerCase(); const candidate=normalizeKey(raw); const explicit=candidate===raw?candidate:''; const key=explicit||(found.key!=='generic'?found.key:providerKey(provider))||'generic'; return {...found,key,mark:MARKS[key]||found.mark||'AI',iconSrcs:local?[ICONS[key]||ICONS.generic]:iconFallbackSrcs(key),theme:themeFor(key,dark)}; }
    root.aiModelVisual={resolve,detect,iconFallbackSrcs,hashHue,themeFor};
