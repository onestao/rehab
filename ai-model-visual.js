...
(function attachAiModelVisual(root) {
    ...
    function iconFallbackSrcs(key='generic'){ const safe=normalizeKey(key)||'generic'; const slug=ICON_SLUGS[safe]||safe; const remote=safe==='kimi'?[`${CDN_BASE}${slug}.svg`]:[`${CDN_BASE}${slug}-color.svg`,`${CDN_BASE}${slug}.svg`]; return [...new Set([...remote,ICONS[safe],ICONS.generic].filter(Boolean))]; }
