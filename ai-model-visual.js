...
(function attachAiModelVisual(root) {
    ...
    const COMPACT_WRAPPER_RE = /^(?:claude|anthropic|openai|gpt|chatgpt|google|gemini|deepseek)(?:[-_\s]+)/i;
    ...
    function resolve({modelId='',provider='',iconKey='',dark,local}={}){ const found=detect(modelId); const candidate=normalizeKey(iconKey); const explicit=(ICONS[candidate]||ICON_SLUGS[candidate])?candidate:''; const key=explicit||(found.key!=='generic'?found.key:providerKey(provider))||'generic'; return {...found,key,mark:MARKS[key]||found.mark||'AI',iconSrcs:local?[ICONS[key]||ICONS.generic]:iconFallbackSrcs(key),theme:themeFor(key,dark)}; }
    function modelLabelCandidates(model={}){
        const id=String(model?.modelId||model?.id||model?.model||'').trim();
        const display=String(model?.displayName||'').trim();
        const custom=!!(display&&id&&display!==id);
        const full=custom?display:(id||display||'模型').replace(/\s+/g,' ');
        const pathPart=full.split('/').pop()||full;
        const compact=custom?full:(pathPart.replace(COMPACT_WRAPPER_RE,'').replace(/[-_\s]+thinking$/i,'').trim()||pathPart);
        return {full,compact,custom,id};
    }
    function compactModelName(model={}){ return modelLabelCandidates(model).compact; }
    root.aiModelVisual={resolve,detect,iconFallbackSrcs,providerKey,hashHue,themeFor,compactModelName,modelLabelCandidates,ICON_SLUGS,ICONS,CDN_BASE};
