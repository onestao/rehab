...
test('manual fallback errors expose only a safe cloned target and preserve the original error', async () => {
  const fallback = {
    profileId: ' backup-profile ', modelId: ' backup-model ', reasoningDepth: 'high',
    apiKey: 'secret', token: 'secret', baseUrl: 'https://secret.invalid', headers: { Authorization: 'secret' }
  };
  const route = { fallbackMode: 'manual', fallbacks: [fallback] };
  const effective = {
    profileId: 'p1', provider: 'openai', modelId: 'primary', fail: true,
    route, enabled: true, apiKey: 'key'
  };
  ...
  await assert.rejects(ai.run({ taskId: 'advice.chat', messages: [] }), error => {
    ...
    assert.deepEqual(JSON.parse(JSON.stringify(error.aiFallback)), {
      taskId: 'advice.chat',
      target: { profileId: 'backup-profile', modelId: 'backup-model' }
    });
    assert.notEqual(error.aiFallback.target, fallback);
    assert.equal(Object.isFrozen(error.aiFallback.target), true);
    assert.doesNotMatch(JSON.stringify(error.aiFallback), /secret|apiKey|token|baseUrl|headers/);
    return true;
  });
});
...
test('manual fallback removes unsafe received targets and exposes no retry for invalid routes', async () => {
  const invalidTargets = [
    { profileId: '', modelId: 'backup' },
    { profileId: 'profile', modelId: 'm'.repeat(257) },
    Object.create({ profileId: 'profile', modelId: 'backup' })
  ];
  for (const target of invalidTargets) {
    const route = { fallbackMode: 'manual', fallbacks: [target] };
    const effective = {
      profileId: 'p1', provider: 'openai', modelId: 'primary', fail: true,
      errorFallback: { target: { profileId: 'attacker', modelId: 'injected', apiKey: 'secret' } },
      route, enabled: true, apiKey: 'key'
    };
    const { ai } = loadAi([effective], route);
    await assert.rejects(ai.run({ taskId: 'advice.chat', messages: [] }), error => {
...
test('non-retryable, automatic and emitted failures never expose manual fallback actions', async () => {
  ...
  const nonRetryable = {
    profileId: 'p1', provider: 'openai', modelId: 'primary', fail: true, status: 400,
    errorFallback: { target: fallback }, route: manualRoute, enabled: true, apiKey: 'key'
  };
  const { ai: nonRetryableAi } = loadAi([nonRetryable], manualRoute);
  nonRetryableAi._callOpenAIChat = async () => {
    const error = Object.assign(new Error('bad request'), { status: 400, aiFallback: { target: fallback } });
    throw error;
  };
  await assert.rejects(nonRetryableAi.run({ taskId: 'advice.chat', messages: [] }), error => {
    assert.equal(error.aiFallback, undefined);
    return true;
  });

  const automaticRoute = { fallbackMode: 'automatic', fallbacks: [fallback] };
  const automatic = { ...nonRetryable, status: 503, route: automaticRoute };
  const { ai: automaticAi } = loadAi([automatic], automaticRoute);
  await assert.rejects(automaticAi.run({ taskId: 'advice.chat', messages: [] }), error => {
    assert.equal(error.aiFallback, undefined);
    return true;
  });

  const streaming = { ...nonRetryable, status: 503, route: manualRoute };
  const { ai: streamAi } = loadAi([streaming], manualRoute);
  streamAi.callStream = async (_messages, _maxTokens, onToken) => {
    onToken('partial', 'partial');
    throw Object.assign(new Error('stream failed'), { status: 503, aiFallback: { target: fallback } });
  };
  await assert.rejects(streamAi.run({ taskId: 'advice.chat', messages: [], stream: true }), error => {
    assert.equal(error.aiFallback, undefined);
    return true;
  });
});
