...
test('old worker passes a newer versioned asset straight to network without touching its cache', async () => {
    ...
    const { request, response } = await harness.dispatch('https://example.test/history-view.js?v=336');

    assert.equal(await response.text(), 'network:https://example.test/history-view.js?v=336');
...
test('worker reports its cache version and release readiness through the startup handshake', async () => {
    const harness = createFetchHarness({ workerVersion: '336' });
    await harness.dispatchMessage({ type: 'GET_VERSION', requestId: 'boot-1' });
    assert.deepEqual(JSON.parse(JSON.stringify(harness.messages)), [{ type: 'VERSION', requestId: 'boot-1', version: '336', precacheReady: false }]);
});

test('upgrade install stays waiting and does not request release assets before an explicit update', async () => {
    const harness = createFetchHarness({ workerVersion: '336', hasActiveWorker: true });
    ...
    assert.ok(harness.adds.includes('build/generated.css?v=336'));
    assert.ok(harness.adds.includes('plan-ui.js?v=336'));
    assert.deepEqual(JSON.parse(JSON.stringify(harness.messages)), [{ type: 'RELEASE_READY', requestId: 'prepare-1', version: '336' }]);
});

test('first install still precaches release assets without forcing a page reload', async () => {
    const harness = createFetchHarness({ workerVersion: '336', hasActiveWorker: false });
    await harness.dispatchInstall();
...
test('legacy v326 SKIP_WAITING prepares before activation, then navigates that old client once', async () => {
    ...
    assert.match(harness.navigations[0].url, /__rehab_upgrade=336/);
...
test('legacy client migrations isolate navigation failures and retain v326 cache until retry succeeds', async () => {
    ...
    assert.match(harness.navigations.find((entry) => entry.clientId === 'legacy-success').url, /__rehab_upgrade=336/);

    harness.setNavigateError('legacy-retry', null);
    const beforeRetry = harness.navigations.length;
    await harness.dispatchMessage({ type: 'V327_PAGE_READY', version: '336' }, harness.client('legacy-success'));
    await harness.waitForMigration();
    ...
    await harness.dispatchMessage({ type: 'V327_PAGE_READY', version: '336' }, harness.client('legacy-retry'));
    await harness.waitForMigration();
...
test('a permanently pending legacy navigation cannot block claim or activation', async () => {
    ...
    await harness.dispatchMessage({ type: 'V327_PAGE_READY', version: '336' }, harness.client('legacy-pending'));
    await harness.waitForMigration();
    assert.deepEqual(harness.pendingLegacyClientIds(), []);
    assert.equal(harness.cacheNames.has('training-assistant-v326'), false);
});

test('a v336 page-ready event acknowledges its queued marker before activation fallback navigation', async () => {
    const harness = createFetchHarness({ workerVersion: '336', hasActiveWorker: true });
    await harness.dispatchMessage({ type: 'SKIP_WAITING' });
    await harness.dispatchMessage({ type: 'V327_PAGE_READY', version: '336' });
    await harness.dispatchActivate();
    await harness.waitForMigration();
...
test('SKIP_WAITING queues every open window client, not only the source tab', async () => {
    ...
    assert.match(harness.navigations.find((entry) => entry.clientId === 'sibling-tab').url, /__rehab_upgrade=336/);
...
test('one ready dual-tab client retains legacy cache and sibling marker', async () => {
    ...
    await harness.dispatchMessage({ type: 'V327_PAGE_READY', version: '336' }, harness.client('client-a'));
    await harness.flush();
...
test('all dual-tab clients ready delete legacy caches and markers', async () => {
    ...
    await harness.dispatchMessage({ type: 'V327_PAGE_READY', version: '336' }, harness.client('client-a'));
    await harness.dispatchMessage({ type: 'V327_PAGE_READY', version: '336' }, harness.client('client-b'));
    await harness.flush();
...
test('gone dual-tab sibling allows legacy cache cleanup after ready peer', async () => {
    ...
    await harness.dispatchMessage({ type: 'EVALUATE_CACHE_CLEANUP', version: '336' }, harness.client('client-a'));
    await harness.flush();
...
test('concurrent cleanup evaluations stay idempotent and safe', async () => {
    ...
    await Promise.all([
        harness.dispatchMessage({ type: 'V327_PAGE_READY', version: '336' }, harness.client('client-a')),
        harness.dispatchMessage({ type: 'EVALUATE_CACHE_CLEANUP', version: '336' }, harness.client('client-a')),
        harness.dispatchMessage({ type: 'EVALUATE_CACHE_CLEANUP', version: '336' }, harness.client('client-b'))
    ]);
    await harness.flush();
    ...
    await Promise.all([
        harness.dispatchMessage({ type: 'V327_PAGE_READY', version: '336' }, harness.client('client-b')),
        harness.dispatchMessage({ type: 'EVALUATE_CACHE_CLEANUP', version: '336' }, harness.client('client-a')),
        harness.dispatchMessage({ type: 'EVALUATE_CACHE_CLEANUP', version: '336' }, harness.client('client-b'))
    ]);
    await harness.flush();
...
test('legacy activation does not leave v326 half-upgraded when release preparation fails', async () => {
    const harness = createFetchHarness({ workerVersion: '336', hasActiveWorker: true, addError: new Error('offline') });
    await harness.dispatchMessage({ type: 'SKIP_WAITING' });
