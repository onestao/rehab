...
test('old worker passes a newer versioned asset straight to network without touching its cache', async () => {
    ...
    const { request, response } = await harness.dispatch('https://example.test/history-view.js?v=327');

    assert.equal(await response.text(), 'network:https://example.test/history-view.js?v=327');
...
test('worker reports its cache version and release readiness through the startup handshake', async () => {
    const harness = createFetchHarness({ workerVersion: '327' });
    await harness.dispatchMessage({ type: 'GET_VERSION', requestId: 'boot-1' });
    assert.deepEqual(JSON.parse(JSON.stringify(harness.messages)), [{ type: 'VERSION', requestId: 'boot-1', version: '327', precacheReady: false }]);
});

test('upgrade install stays waiting and does not request release assets before an explicit update', async () => {
    const harness = createFetchHarness({ workerVersion: '327', hasActiveWorker: true });
    ...
    assert.ok(harness.adds.includes('build/generated.css?v=327'));
    assert.ok(harness.adds.includes('plan-ui.js?v=327'));
    assert.deepEqual(JSON.parse(JSON.stringify(harness.messages)), [{ type: 'RELEASE_READY', requestId: 'prepare-1', version: '327' }]);
});

test('first install still precaches release assets without forcing a page reload', async () => {
    const harness = createFetchHarness({ workerVersion: '327', hasActiveWorker: false });
    await harness.dispatchInstall();
...
test('legacy v326 SKIP_WAITING prepares before activation, then navigates that old client once', async () => {
    ...
    assert.match(harness.navigations[0].url, /__rehab_upgrade=327/);
...
test('a v327 page-ready event acknowledges its queued marker before activation fallback navigation', async () => {
    const harness = createFetchHarness({ workerVersion: '327', hasActiveWorker: true });
    await harness.dispatchMessage({ type: 'SKIP_WAITING' });
...
test('legacy activation does not leave v326 half-upgraded when release preparation fails', async () => {
    const harness = createFetchHarness({ workerVersion: '327', hasActiveWorker: true, addError: new Error('offline') });
    await harness.dispatchMessage({ type: 'SKIP_WAITING' });
